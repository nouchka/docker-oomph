...
RUN apt-get update && \
	DEBIAN_FRONTEND=noninteractive apt-get -yq install wget git libcanberra-gtk3-module ssh-askpass openssh-client && \
	...
	[ "$DOCKER_TAG" = "latest" ] || wget -qO- https://getcomposer.org/installer | php -- --install-dir=/usr/local/bin --filename=composer && \
